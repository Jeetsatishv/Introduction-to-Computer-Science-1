...
def inner_grid(height, width, digit):
    '''This function creates and returns a 2-D list of height rows and width columns 
    in which the “inner” cells all have a value of digit–which is an integer between 0 and 9 
    inclusive–and the cells on the outer border are all 0'''
...
def copy(grid):
    '''This function creates and returns a deep copy of grid–a new, separate 2-D 
    ist that has the same dimensions and cell values as grid'''
...
def add_one(grid):
    '''This function takes an existing 2-D list of digits and adds 1 
...
def negate_bits(grid):
    '''This function takes an existing 2-D list of binary digits (0s and 1s) 
    and negates all of the bits – changing the 0s to 1s and the 1s to 0s.'''
    for r in range(len(grid)):
        for c in range(len(grid[0])):
            if grid[r][c]==0:
                grid[r][c]=1
