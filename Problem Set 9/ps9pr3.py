...
def process_move(p, b):
    '''This function takes two parameters: a Player object p for the 
    player whose move is being processed, and a Board object b for the 
...
class RandomPlayer(Player):
    '''This class can be used for an unintelligent computer player 
    that chooses at random from the available columns.'''
    def next_move(self, b):
        self.num_moves += 1
        available_slots = [j for j in range(b.width) if b.can_add_to(j)]
